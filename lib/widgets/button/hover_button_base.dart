  ...
  Widget build(BuildContext context) {
    ...
    w = widget.focusEnabled
        ? FocusableActionDetector(
            ...
            onShowFocusHighlight: (v) => {if (mounted) setState(() => _shouldShowFocus = v)},
            onShowHoverHighlight: (v) => {if (mounted) setState(() => _hovering = v)},
            child: w,
          )
        : MouseRegion(
            cursor: widget.cursor ?? MouseCursor.defer,
            onEnter: (e) => {if (mounted) setState(() => _hovering = true)},
            onExit: (e) => {if (mounted) setState(() => _hovering = false)},
            child: w,
          );
    w = MergeSemantics(
      child: Semantics(
    ...
    return _HoverButtonInherited(states: states, child: w);
  }
...
class _AllButtonState<T> extends ButtonState<T> {
  ...
  @override
  T resolve(Set<ButtonStates> states) => _value;
}

class _LerpProperties<T> implements ButtonState<T?> {
  const _LerpProperties(this.a, this.b, this.t, this.lFunction);

  final ButtonState<T?>? a;
  final ButtonState<T?>? b;
  final double t;
  final T? Function(T?, T?, double) lFunction;
  ...
  T? resolve(Set<ButtonStates> states) {
    ...
    return lFunction(resolvedA, resolvedB, t);
  }
}
